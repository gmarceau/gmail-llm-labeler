...
class TestDumpDomains:
    ...
    def test_gmail_fetched_for_missing_emails(self, tmp_path, capsys):
        ...
        def fake_backfill(processor, email_ids, db_):
            db_.save_email("missing-id", "Hello", "sender@remote.com", "2024-01-01", "",
                           {"list-unsubscribe": "<url>"})

        with patch("email_labeler.pipeline.cli.PipelineConfig.from_yaml") as mock_cfg, \
             patch("email_labeler.pipeline.cli.PathConfig") as mock_pc, \
             patch("email_labeler.pipeline.cli.EmailDatabase", return_value=db), \
             patch("email_labeler.pipeline.cli.EmailProcessor"), \
             patch("email_labeler.pipeline.cli.backfill_email_metadata",
        ...
        assert "missing-id" in email_ids_arg
        output = json.loads(capsys.readouterr().out)
